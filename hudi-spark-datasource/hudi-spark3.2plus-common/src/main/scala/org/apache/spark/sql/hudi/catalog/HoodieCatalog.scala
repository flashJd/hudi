...
class HoodieCatalog extends DelegatingCatalogExtension
  ...
  def createHoodieTable(ident: Identifier,
    ...
    if (operation == TableCreationMode.STAGE_CREATE) {
      ...
      saveSourceDF(sourceQuery, tableDesc.properties ++ buildHoodieInsertConfig(hoodieCatalogTable, spark, overwrite = false, Map.empty, options))
      ...
      saveSourceDF(sourceQuery, tableDesc.properties ++ buildHoodieInsertConfig(hoodieCatalogTable, spark, overwrite = false, Map.empty, Map.empty))
      new CreateHoodieTableCommand(tableDesc, false).run(spark)
    }
