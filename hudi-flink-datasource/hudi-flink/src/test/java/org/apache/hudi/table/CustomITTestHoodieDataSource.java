 ...
 */
public class CustomITTestHoodieDataSource extends AbstractTestBase {
  private TableEnvironment streamTableEnv;
  private TableEnvironment batchTableEnv;

  @BeforeEach
  void beforeEach() throws IOException {
    tempFile = new File("./hudi_table1");
  ...
  void testWriteAndReadWithProctimeSequence(HoodieTableType tableType, boolean hiveStylePartitioning) {
    ...
    String hoodieTableDDL = sql("t1")
        ...
        .field("age int")
        .field("ts timestamp(3)") // use a different field with default precombine field 'ts'
  ...
  void testWriteAndReadParMiddle(ExecMode execMode) throws Exception {
    ...
    String insertInto = "insert into t1 values\n"
        ...
        + "('id1','Danny',24,'par4',TIMESTAMP '1970-01-01 00:00:08')";
    execInsertSql(streamTableEnv, insertInto);
    ...
    List<Row> result = execSelectSql1(streamTableEnv, "t1", "select * from t1", execMode);
  ...
  private List<Row> execSelectSql1(TableEnvironment tEnv, String tableName, String select, ExecMode execMode)
          throws TableNotExistException, InterruptedException {
    switch (execMode) {
      case STREAM:
        return execSelectSql(tEnv, select, 10, tableName);
