...
public class TestOnLineCleanArchive {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // execConf.setString("execution.checkpointing.unaligned", "true");
    ...
    tableDDL();
    // String insertInto = "insert into t1 select * from
    // default_catalog.default_database.datagen";
    String insertInto =
  ...
  private static void tableDDL() {
    String hoodieTableDDL =
        sql("t1")
            ...
            .option("compaction.delta_commits", "3")
  ...
  private static Long getRecordNum(
    ...
    if (metadataConfig.enabled() && metadataConfig.isColumnStatsIndexEnabled()) {
      ...
      List<String> encodedTargetColumnNames =
          Arrays.stream(new String[] {column})
      ...
      HoodieData<Long> columnStats =
          records
                    ...
                    try {
                      metadataRecord =
                          (HoodieMetadataRecord)
                              record.getData().getInsertValue(null, null).orElse(null);
              ...
              .filter(stat -> baseFilePaths.contains(stat.getFileName()))
              .map(HoodieMetadataColumnStats::getValueCount);
                    ...
                    try {
                      HadoopInputFile file =
                          HadoopInputFile.fromPath(
                              inputFile, new org.apache.hadoop.conf.Configuration());
  ...
  void statTable() {
    ...
    String[] partitions =
        FSUtils.getAllPartitionPaths(HoodieFlinkEngineContext.DEFAULT, metadataConfig, basePathStr)
    ...
    FileStatus[] allFileStatus =
        FSUtils.getFilesInPartitions(
                ...
                Arrays.stream(partitions)
                    .map(p -> fullPartitionPath(new Path(basePathStr), p))
                    .toArray(String[]::new),
                "/tmp")
            .values().stream()
    ...
    long totalSize =
        Arrays.stream(allFileStatus).map(FileStatus::getLen).reduce(Long::sum).orElse(0L);
    System.out.println("-----Table totalSize: " + totalSize);
    System.out.println("-----Table total files: " + allFileStatus.length);

    long smallFileNum =
        Arrays.stream(allFileStatus)
            .map(FileStatus::getLen)
            .filter(len -> len <= smallFileSize)
            .count();
    ...
    HoodieTableFileSystemView fileSystemView =
        FileSystemViewManager.createInMemoryFileSystemView(
            HoodieFlinkEngineContext.DEFAULT,
            HoodieTableMetaClient.builder().setConf(conf).setBasePath(basePathStr).build(),
            metadataConfig);

    List<HoodieBaseFile> baseFiles =
        Arrays.stream(partitions)
            .flatMap(fileSystemView::getLatestBaseFiles)
            .collect(Collectors.toList());
