...
public class TestOnLineCleanArchive {
  ...
  void readTable() {
    ...
    List<Row> rows = CollectionUtil.iterableToList(
        () -> streamTableEnv.sqlQuery("select * from t1 limit 10").execute().collect());
  ...
  private static void tableDDL() {
    String hoodieTableDDL =
        sql("t1")
            ...
            .option("hoodie.metadata.index.async", "true")
            .option("hoodie.metadata.index.async.incremental", "true")
